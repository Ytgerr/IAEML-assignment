...
class VehicleDynamics:
    ...
    def step_dynamics(state, action, dt, friction_coeff=0.1):
        ...
        steering_rate = jnp.clip(steering_rate, -10.0, 10.0)
        delta = jnp.clip(delta, -0.25, 0.25)

        new_delta = delta + steering_rate * dt
        new_delta = jnp.clip(new_delta, -1.0, 1.0)
        ...
        new_state = jax.lax.cond(
            is_straight, straight_motion, turning_motion, None
        )
...
class RobotaxiObservation(BaseEnvObservation):
    ...
    goal_distance: jnp.ndarray
    goal_angle: jnp.ndarray
...
class RobotaxiEnv(BaseEnv):
    ...
    def init_params(
        ...
        initial_vehicle_state = jnp.array(
            [
                ...
                jnp.arctan2(
                    base_state.agent_forward_dir[1],
                    base_state.agent_forward_dir[0],
                ),
                0.0,
                0.0,
            ]
    ...
    def step(
        ...
        new_agent_forward_dir = jnp.array(
            [jnp.cos(new_vehicle_state[2]), jnp.sin(new_vehicle_state[2])]
        )
        ...
        goal_done = RobotaxiEnv._check_goal(
            new_agent_pos, env_state.goal_pos
        )
        collision_static = RobotaxiEnv._check_collisions(
            new_agent_pos, env_state.static_obstacles
        )
        collision_dynamic = RobotaxiEnv._check_collisions(
            new_agent_pos, new_kinematic_obstacles
        )
        time_done = env_state.time >= env_params.max_steps_in_episode
            ...
            jnp.logical_or(
                collision_static,
                jnp.logical_or(collision_dynamic, time_done),
            ),
        )
    ...
    @staticmethod
    def get_observation(
        env_state: RobotaxiState, env_params: BaseEnvParams
    ) -> RobotaxiObservation:
    ...
    def _compute_reward(
        ...
        goal_reward = jax.lax.cond(
            goal_done, lambda _: 300.0, lambda _: 0.0, None
        )

        static_penalty = jax.lax.cond(
            collision_static, lambda _: -200.0, lambda _: 0.0, None
        )

        dynamic_penalty = jax.lax.cond(
            collision_dynamic, lambda _: -200.0, lambda _: 0.0, None
        )
        ...
        progress_reward = (
            old_goal_dist
            - jnp.linalg.norm(new_agent_pos - old_state.goal_pos)
        ) * 15.0

        action_penalty = -0.001 * jnp.sum(
            jnp.square(old_state.last_action)
        )

        speed = new_vehicle_state[3]
        movement_reward = speed * 0.5

        stillness_penalty = jax.lax.cond(
            speed < 0.01, lambda _: -2.0, lambda _: 0.0, None
        )
